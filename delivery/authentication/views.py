...
class DeliveryPersonLogoutView(APIView):
    permission_classes = [IsAuthenticated]  # Changed from AllowAny to ensure only authenticated users can logout

    def post(self, request):
        try:
            
            person = request.user.delivery_person_profile
            person.is_available = False
            person.save()
            if  person.assigned_orders:
                return api_response(
                message =  "Cannot Logout. An Order is assigned to you",
                status_code=status.HTTP_403_FORBIDDEN
            )
